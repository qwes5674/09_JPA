...
public class EmbeddedKeyTests {
    ...
    * 특징 : 두 방식 모두 복합키 클래스 -> 영속성 컨텍스트가 관리하지 않는다는 특징을 가진다.
    *
    * 주의할 점
    ...
    * */
    ...
    public void embeddedIdTest(){
        ...
        //then
        Member foundmember = entityManager.find(Member.class, member.getMemberPk()); //found -> DB안에 있는 데이터를 찾는다
        assertEquals(member.getMemberPk(), foundmember.getMemberPk()); // 특정 데이터를 찾을때 PK
