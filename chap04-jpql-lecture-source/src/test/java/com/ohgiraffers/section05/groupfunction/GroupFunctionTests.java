...
public class GroupFunctionTests {
    ...
    /*
    * 그룹합수
    * -> JPQL에서 사용하는 그룹함수량, 기존 SQL에서 사용하는 그룹함수는 차이가 거의 없다.
    * COUNT, SUM, AVG, MAX, MIN
    *
    * 주의사항
    * 1. 그룹함수의 반환 타입은 결과 값이 정수면 Long, 실수면 Double을 반환된다.
    * 2. 값이 없는 상태에서 COUNT를 제외한 그룹함수는 null이 되고 COUNT는 0이 된다.
    *    -> null로 받은 상태에서는 long 또는 double 가은 자료형으로 받게되면
    *       언박싱 과정에서 NPE가 발생한다.
    * 3. 그룹함수의 반환 자료형은 Long or Double 형이기 떄문에 Having절에서
    ...
    * */
    ...
    @Test
    @DisplayName("특정 카테고리의 등록된 메뉴 수 조합")
    void test1(){
        //given
        ...
        String jqpl = """
                ...
                """;
        long countOfMenu = entityManager.createQuery(jqpl, Long.class)
    ...
    @Test
    @DisplayName("count를 제외한 다른 그룹함수의 조희결과가 없는 경우 테스트")
    void test2() {
        //given
...
//        });
        ...
        assertDoesNotThrow(() -> {
            long sumOfMenu = entityManager.createQuery(jqpl, Long.class)
                    .setParameter("categoryCode", categoryCodeParameter)
                    .getSingleResult();
        });
    }
